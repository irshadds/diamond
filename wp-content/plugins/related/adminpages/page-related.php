		...
		if ( $_POST['form'] == 'related_show' ) {
			...
			if ( $verified == false ) {
				...
				if ( isset( $_POST['related_uninstall_confirmed'] ) && $_POST['related_uninstall_confirmed'] == 'on' && isset( $_POST['related_uninstall'] )  ) {
					...
					$dashboard = '<a href="' . admin_url( '/index.php' ) . '">' . esc_html__('Dashboard', 'realted') . '</a>';
					...
					echo '
						<div id="message" class="updated fade notice is-dismissible">
							...
							<p>' . sprintf( __('You can now go to your %s.', 'related'), $dashboard ) . '</p>
						</div>
						';
				} else {
	...
	<div role="tabpanel" class="related_options related_uninstall <?php if ($active_tab == 'related_uninstall') { echo "active";} ?>">
		<div class="poststuff metabox-holder">
			<div class="related-widget">
				...
				_e('This can <strong>not</strong> be undone.', 'related'); echo '<br />';
				...
				<div class="misc-pub-section">
					<form name="related_options_page_uninstall" action="" method="POST">
						...
						echo '<input type="hidden" id="related_uninstall_nonce" name="related_uninstall_nonce" value="' . $nonce . '" />'; ?>
		...
		<div class="poststuff metabox-holder">
			<div class="related-widget">
				...
				<p><?php
					...
					echo sprintf( esc_html__( 'If you have a problem or a feature request, please post it on the %ssupport forum at wordpress.org%s.', 'related' ), $support, '</a>' ); ?>
				...
				<p><?php
					...
					/* translators: %s is a link */
					echo sprintf( esc_html__( 'If this plugin has any value to you, then please leave a review at %sthe plugin page%s at wordpress.org.', 'related' ), $review, '</a>' ); ?>
				</p>
				...
				<p><?php
					...
					/* translators: %s is a link */
					echo sprintf( esc_html__( 'Translations can be added very easily through %sGlotPress%s.', 'related' ), $link, '</a>' ); echo '<br />';
					echo sprintf( esc_html__( "You can start translating strings there for your locale. They need to be validated though, so if there's no validator yet, and you want to apply for being validator (PTE), please post it on the %ssupport forum%s.", 'related' ), $support, '</a>' ); echo '<br />';
					$make = '<a href="https://make.wordpress.org/polyglots/" target="_blank">';
					/* translators: %s is a link */
					echo sprintf( esc_html__( 'I will make a request on %smake/polyglots%s to have you added as validator for this plugin/locale.', 'related' ), $make, '</a>' ); ?>
				</p>
